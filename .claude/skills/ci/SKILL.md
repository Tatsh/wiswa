...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `wiswa/`, `tests/`,
...
`wiswa/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `wiswa/media.py` and `.claude/agents/release.md`).

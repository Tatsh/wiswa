...
/**
 * @brief Default Python dependency definitions in Poetry-style syntax.
 *
...
function(want_main, want_yapf, stubs_only, project_name, want_coveralls, want_sqlfluff, want_ty,
 min_python_version='3.10') {
  ...
  local cz_path_dep = if project_name != 'cz-path' then { 'cz-path': ver('cz-path') } else {},
  main: {
  ...
  tests: {
    mock: ver('mock'),
    pytest: ver('pytest'),
    'pytest-cov': ver('pytest-cov'),
    'pytest-mock': ver('pytest-mock'),
  } + if want_coveralls then { coveralls: ver('coveralls') } else {},
}
